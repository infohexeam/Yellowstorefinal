	...
	public function listOrder(Request $request)
	{
		...
		if ($_GET) {
			...
			if ($store_id == 0 && isset($subadmin_id)) {
				...
				$store_data = DB::table('trn_store_orders')->select("store_id")->where('subadmin_id', '=',$subadmin_id)->where('subadmin_id', '!=',NULL)->get();
				$store_array[] = 0;
				...
				$store_data = \DB::table('trn_store_orders')->select("store_id")->get();
				foreach ($store_data as $val) {
					$store_array[] = $val->store_id;
				}
				$query = $query->whereIn('store_id', $store_array);

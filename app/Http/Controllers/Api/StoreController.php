    ...
    public function customerVideoList(Request $request)
    {
        ...
        try {
            ...
            if (isset($request->customer_id)) {
                ...
                $cusVids = $cusVids->where('town_id', $cust->town_id)->where('state_id', $cust->state_id)->where('district_id', $cust->district_id);//->orWhereNull('town_id');
            
            } else {
                $cusVids = $cusVids->whereNull('town_id')->whereNull('state_id')->whereNull('district_id');
    ...
    public function customerVideoListNew(Request $request)
{
    ...
    try {
        ...
        // Add conditions to match state_id and district_id
        $cusVids = $cusVids
            ->where(function ($query) use ($cust) {
                $query
                    ->where('state_id', $cust->state_id)
                    ->orWhereNull('state_id');
            })
            ->where(function ($query) use ($cust) {
                $query
                    ->where('district_id', $cust->district_id)
                    ->orWhereNull('district_id');
            })
            ->where(function ($query) use ($cust) {
                $query
                    ->where('town_id', $cust->town_id)
                    ->orWhereNull('town_id');
            });
    } else {
        // If store_id is not provided, match records where state_id, district_id, and town_id are null
...
$commission_order_numeric = is_numeric($sd->commission_order) ? (float) $sd->commission_order : 0;
    ...
    public function walletReport(Request $request)
	{
      ...
      foreach($walletdata as $w)
      {
        ...
        $w->amount_before_applying_rp=strval(number_format($item_price+$w->delivery_charge+$w->packing_charge-$w->amount_reduced_by_coupon,2));
    ...
    public function getStoreReferrals(Request $request)
    {
        try{
            ...
            ->where('store_id',$store_id)->where('reference_status',1)->orderBy('trn_store_referrals.created_at','DESC')->get();
            return response($data);

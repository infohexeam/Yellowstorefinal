...
class StoreController extends Controller
{
            ...
            if (isset($request->store_id) && Mst_store::find($request->store_id)) {
                ...
                $inventoryData =   Mst_store_product_varient::leftjoin('mst_store_products', 'mst_store_products.product_id', '=', 'mst_store_product_varients.product_id')
                    ...
                    ->select(
                        ...
                        'mst_store_product_varients.updated_at AS updated_time',
    ...
    public function walletReport(Request $request)
	{
      ...
      foreach($walletdata as $w)
      {
        ...
        $w->amount_before_applying_rp=strval(number_format($item_price+$w->amount_reduced_by_rp??0+$w->packing_charge??0+$w->delivery_charge??0+$w->amount_reduced_by_rp_store??0+$w->amount_reduced_by_coupon??0,2));

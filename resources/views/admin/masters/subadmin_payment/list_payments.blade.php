                           ...
                           <table id="exampletable" class="table table-striped table-bordered text-nowrap w-100">
                              <thead>
                                 <tr>
                                    ...
                                    <th class="wd-15p">Commision<br>Percentage</th>
                                    <th class="wd-15p">{{ __('Total') }}</th>
                                     <th class="wd-20p">SubAdmin<br>Commision</th>
                                    {{-- <th class="wd-20p">Delivery Boy</th> --}}
                                    {{-- <th class="wd-20p">Amount Paid</th>
                                    <th class="wd-15p">Amount<br></bt>to be Paid</th>
                              ...
                              <tbody>
                                 ...
                                 <tr>
                                    ...
                                    <td>{{ @$payment->commision_percentage }}</td>
                                    <td>{{ @$payment->order['product_total_amount']}}</td>
                                    @php
                                    $sub_comm = (@$payment->commision_percentage / 100) * @$payment->order['product_total_amount'];
                                    $total_order_amount = $total_order_amount + @$payment->order['product_total_amount'];
                                    @endphp
                                    <td>{{ @$sub_comm }}</td>
                                   </td>
                                 </tr>
                            ...
                            <div class="row">
                                <div class="col-12" >
                                @php
                                      $sub_admin = \DB::table('mst_subadmin_details')->where('subadmin_id',$subadmin_id)->first();

                                    $balance_query = \DB::table('trn_subadmin_payments_tracker')->where('subadmin_id',$subadmin_id);
                                      ...
                                      if(request()->input('year') && request()->input('month'))
                                      {
                                        ...
                                        $balance = $balance_query->whereBetween('date_of_payment',[$a1,$a2]);
                                      }
                                    $paid = $balance_query->sum('commision_paid');
                                    $effective_balance_amount = @$commision_pay + @$sub_admin->subadmin_commision_amount;
                                    $amount_to_be_paid =  $effective_balance_amount - $paid;
                                @endphp
                                   <h4>Sub Admin Fixed Amount: <b> Rs. {{@$sub_admin->subadmin_commision_amount}}</b></h4>
                                </div>
                                 <div class="col-12" >
                                   <h4>Total Order Amount: <b> Rs. {{  @$total_order_amount }}</b></h4>
                                </div>
                                <div class="col-12" >
                                   <h4>Sub Admin Total Commission [{{$sub_admin->subadmin_commision_percentage }}% of total order amount]: <b> Rs. {{@$commision_pay}}</b></h4>
                                </div>
                                 <div class="col-12" >
                                   <h4>Effective Balance Amount: <b> Rs. {{ @$effective_balance_amount }}</b></h4>
                                </div>
                                <div class="col-12" >
                                 <h4>  Paid Amount: <b> Rs. {{ @$paid }} </b> </h4>
                                </div>
                                <div class="col-12" >
                            ...
                            <div class="row">
                              <div class="col-4" >
                                   @if(auth()->user()->user_role_id == 0 )
                                     <a data-toggle="modal" data-target="#StockModal{{$subadmin_id}}" class="btn btn-small btn-success">
...
@endif        
            ...
            <div class="modal fade" id="StockModal{{$subadmin_id}}" tabindex="-1" role="dialog"  aria-hidden="true">
               <div class="modal-dialog" role="document">
                  ...
                  <form action=" {{ url('admin/subadmin/pay/'.Crypt::encryptString($subadmin_id)) }} " method="POST" enctype="multipart/form-data" >
                 @csrf

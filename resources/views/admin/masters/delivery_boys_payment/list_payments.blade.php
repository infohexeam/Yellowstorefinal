...
<div class="container">
   ...
   @if(request()->input('year') && request()->input('month'))
        @php
          ...
          @$d_count = \DB::table('mst_delivery_boys')
                     ...
                     <div class="card-body">
                        ...
                        <div class="table-responsive">
                           <table id="exampletable" class="table table-striped table-bordered text-nowrap w-100">
                            ...
                            <td>{!! date('d/M/y', strtotime(@$delivery_boy_payment->created_at)) !!}</td>
                                    <td>{{ @$delivery_boy_payment->order['product_total_amount'] }}</td>
                                 ...
                                 @php
                                     @$commision_pay =  $commision_pay + $delivery_boy_payment->delivery_boy_commision_amount;
                                    $total_order_amount = $total_order_amount + @$delivery_boy_payment->order['product_total_amount'] ;
                                 @endphp
                                 @endforeach
                              </tbody>
                           </table>
                            ...
                            <div class="row">
                             <div class="col-12" >
                             @php
                                    $dboys = \DB::table('mst_delivery_boys')->where('delivery_boy_id',$delivery_boy_id)->first();

                                    $balance_query = \DB::table('trn_delivery_boy_payments')->where('delivery_boy_id',$delivery_boy_id);
                                        if(request()->input('year') && request()->input('month'))
                                        {
                                             $year = request()->input('year');
                                                $month = request()->input('month');
                                        ...
                                        $delivery_boy_total_amount = @$i * @$dboys->delivery_boy_commision;

                                        $effective_balance_amount = $delivery_boy_total_amount + @$dboys->delivery_boy_commision_amount;

                                        $amount_to_be_paid = $effective_balance_amount - @$paid_amount;

                             @endphp
                                   <h4>Delivery Boy Fixed Amount: <b> Rs. {{@$dboys->delivery_boy_commision_amount}}</b></h4>
                                </div>

                                <div class="col-12" >
                                   <h4>Total Order Amount: <b> Rs. {{@$total_order_amount}}</b></h4>
                                </div>

                                 <div class="col-12" >
                                   <h4>Total Number of Orders Delivered: <b> Rs. {{@$i}}</b></h4>
                                </div>

                                <div class="col-12" >
                                   <h4>Delivery Boy Total Commission [Rs.{{ @$dboys->delivery_boy_commision}}/- per Order]: <b> Rs. {{@$delivery_boy_total_amount}}</b></h4>
                                </div>
                            ...
                            <div class="row">
                              <div class="col-6" >
                                    ...
                                    <a data-toggle="modal" data-target="#StockModal{{$delivery_boy_id}}" class="btn btn-small btn-success">
...
@endif
            ...
            <div class="modal fade" id="StockModal{{$delivery_boy_id}}" tabindex="-1" role="dialog"  aria-hidden="true">
               <div class="modal-dialog" role="document">
                 ...
                 <form action=" {{ url('admin/delivery_boy/pay/'.Crypt::encryptString($delivery_boy_id)) }} " method="POST" enctype="multipart/form-data" >
                 @csrf
